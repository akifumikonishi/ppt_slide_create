...
slide_data = [
    ...
    },
    {
        ...
        'notes': '本調査では、2024年から2025年にかけてのSuccessFactors導入事例を詳細に分析しました。特に、日本本社で使用中のSuccessFactorsに南アフリカ子会社のシステムを統合する際の特有のリスクに焦点を当てています。'
    },
    {
        'type': 'compare',
        'title': '主要リスクカテゴリー分析',
        ...
        'notes': '技術的リスクでは、2025年に予定されている大規模なシステム変更への対応が急務となっています。特に認証システムの移行とAPI変更は必須対応項目です。'
    },
    {
        'type': 'timeline',
        ...
        'notes': '2025年は SuccessFactors にとって大きな変革の年となります。これらの変更は必須対応であり、期限内に対応しない場合、システムアクセスができなくなるリスクがあります。'
    },
    {
        'type': 'cards',
        'title': '統合成功のための対処策',
        ...
        'notes': '成功事例から抽出された対処策では、特に段階的移行戦略の重要性が強調されています。データ移行プロジェクトの83%が失敗または予算・スケジュール超過していることから、これらの対策は必須と考えられます。'
    },
    {
        'type': 'closing',
        'notes': '本調査により、SuccessFactorsグローバルロールアウトにおける主要なリスクと効果的な対処策が明確になりました。特に2025年の必須対応項目への早期着手と、段階的移行戦略の採用が成功の鍵となります。'
    }
]

def set_font(paragraph_or_run, font_name="Meiryo UI", font_size=None):
    """フォント設定を統一する関数"""
...
def calculate_font_size(text, max_width, max_height, base_size):
    """テキスト長に基づいて適切なフォントサイズを計算"""
    text_length = len(text)
    if text_length > 200:
        return Pt(max(base_size - 6, 10))
    elif text_length > 150:
        return Pt(max(base_size - 4, 12))
    elif text_length > 100:
        return Pt(max(base_size - 2, 14))
    else:
        return Pt(base_size)
...
def create_title_slide(prs, slide_data):
    ...
    # タイトル
    title_box = slide.shapes.add_textbox(Inches(1), Inches(3), Inches(14), Inches(2.5))
    ...
    set_font(title_frame.paragraphs[0])
    ...
    set_font(date_frame.paragraphs[0])
...
def create_content_slide(prs, slide_data):
    ...
    title_frame = title_box.text_frame
    title_frame.text = slide_data['title']
    title_font_size = calculate_font_size(slide_data['title'], 15, 1, 26)
    ...
    if 'subhead' in slide_data:
        subhead_box = slide.shapes.add_textbox(Inches(0.5), Inches(1.5), Inches(15), Inches(0.7))
        subhead_frame = subhead_box.text_frame
        subhead_frame.text = slide_data['subhead']
        subhead_frame.paragraphs[0].font.size = Pt(16)
        subhead_frame.paragraphs[0].font.color.rgb = RGBColor(102, 102, 102)
        set_font(subhead_frame.paragraphs[0])
        subhead_frame.word_wrap = True
    
    # ポイント
    if 'points' in slide_data:
        points_box = slide.shapes.add_textbox(Inches(0.5), Inches(2.5), Inches(15), Inches(5.5))
        ...
        for i, point in enumerate(slide_data['points']):
            ...
            p.text = f"• {point}"
            point_font_size = calculate_font_size(point, 15, 1, 18)
...
def create_compare_slide(prs, slide_data):
    ...
    # 左側タイトル
    left_title_box = slide.shapes.add_textbox(Inches(0.5), Inches(2.5), Inches(7), Inches(0.5))
    ...
    set_font(left_title_frame.paragraphs[0])
    
    # 右側タイトル
    right_title_box = slide.shapes.add_textbox(Inches(8.5), Inches(2.5), Inches(7), Inches(0.5))
    ...
    set_font(right_title_frame.paragraphs[0])
    
    # 左側アイテム
    left_box = slide.shapes.add_textbox(Inches(0.5), Inches(3.2), Inches(7), Inches(5))
    ...
    for i, item in enumerate(slide_data['left_items']):
        ...
        p.text = f"• {item}"
        item_font_size = calculate_font_size(item, 7, 1, 14)
        p.font.size = item_font_size
        p.space_after = Pt(6)
        set_font(p)
    
    # 右側アイテム
    right_box = slide.shapes.add_textbox(Inches(8.5), Inches(3.2), Inches(7), Inches(5))
...
def create_timeline_slide(prs, slide_data):
    ...
    for milestone in slide_data['milestones']:
        ...
        # 日付
        date_box = slide.shapes.add_textbox(Inches(0.5), Inches(y_pos), Inches(3), Inches(0.6))
        date_frame = date_box.text_frame
        ...
        # ラベル
        label_box = slide.shapes.add_textbox(Inches(4), Inches(y_pos), Inches(11.5), Inches(0.6))
        ...
        set_font(label_frame.paragraphs[0])
...
def create_cards_slide(prs, slide_data):
    ...
    for i, item in enumerate(slide_data['items']):
        ...
        x = 0.5 + col * (card_width + 0.7)
        y = 2.6 + row * (card_height + 0.3)
        ...
        if isinstance(item, dict):
            ...
            set_font(text_frame.paragraphs[0])
...
def create_closing_slide(prs, slide_data):
    ...
    thanks_frame.paragraphs[0].font.size = Pt(44)
    ...
    set_font(thanks_frame.paragraphs[0])
